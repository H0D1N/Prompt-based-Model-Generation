...
def onnxint2ndtype(val: int):
    ...
    return match_dict[val]


def inout_gen(model_path):
    """summary:
        generate input data and output names for required onnx model

    Args:
        model_path (str): path of onnx model

    Raises:
        Exception: undefined model path or data type
    Returns:
        input_data (dict): input data for onnx model
        output_names (list): output names for required onnx model
    """
    model = onnx.load(model_path)
    # input 和 output的类型是<class 'google.protobuf.pyext._message.RepeatedCompositeContainer'>
    """
    input
    [name: "input_1:0"
        type {
    ...
    input_data = {}
    for input in inputs:
        tp = onnxint2ndtype(input.type.tensor_type.elem_type)
        if tp == None:
            # raise exception
            raise Exception("Undefined data type. Exit.")

        else:
            shape = [val.dim_value for val in input.type.tensor_type.shape.dim]
            """
            对于            
            dim {
                dim_param: "unk__2104"
            }中没有dim_value的情况, 将其设置为1
            """
            shape = list(map(lambda x: 1 if x == 0 else x, shape))
            input_data[input.name] = np.ones(shape=shape, dtype=tp)
    print(output_names)
