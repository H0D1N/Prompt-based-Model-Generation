...
#print(np.array(peak_memory_footprint)[:400])


model_memory_train = np.array(peak_memory_footprint[:slice_num]).reshape(slice_num, 1)  # 将前400个元素作为训练集

model_memory_valid = np.array(peak_memory_footprint[slice_num:]).reshape(runs - slice_num, 1)  # 将后100个元素作为测试集
...
plt.text(x_coord + 100, y_coord, f'acc std: {model_memory_std_acc:.3f}', fontsize=12)
...
np.savetxt('../../box_plot/box_plot_data/jetson_gpu_model_memory_acc.txt', model_memory_acc)
