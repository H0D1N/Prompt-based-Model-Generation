...
class HeadNet(nn.Module):
    ...
    @torch.jit.ignore()
    def _forward(self, x: List[torch.Tensor],decision,bias) -> List[torch.Tensor]:
        outputs = []
        decision_list=[decision[:,i,:] for i in range(3)]
        bias_list=[bias[:,i,:] for i in range(3)]
        for level in range(self.num_levels):
            x_level = x[level]
            for conv, bn,gate ,b in zip(self.conv_rep, self.bn_rep,decision_list,bias_list):
                x_level = conv(x_level)
                x_level=x_level+b[:,:conv.conv.out_channels,None,None]
                x_level = bn[level](x_level)# this is not allowed in torchscript
                x_level = self.act(x_level)
                x_level=x_level*gate[:,:conv.conv.out_channels,None,None]
    ...
    def forward(self, x: List[torch.Tensor],decision,bias) -> List[torch.Tensor]:
        if self.bn_level_first:
            return self._forward_level_first(x)
        else:
            return self._forward(x,decision,bias)
...
class EfficientDet(nn.Module):
    ...
    def forward_bottleneck(self, layer, x, kernel_selection=None,bias=None):
        shortcut = x
        if kernel_selection == None:
            ...
            x = layer.conv1(x)
            x=x+bias[:, 0, :][:, :layer.conv1.out_channels, None,
                    None]
            ...
            x = layer.conv2(x)
            x=x+bias[:, 1, :][:, :layer.conv2.out_channels, None,
                    None]
            x = layer.bn2(x)
    ...
    def forward_backbone(self, x, decision=None,bias=None):
        output=[]
        ...
        for layer in self.backbone.layer1:
            ...
            x = self.forward_bottleneck(layer, x, decision[:, gate_num:gate_num + 2, :],bias[:, gate_num:gate_num + 2, :])
            gate_num += 2
        ...
        for layer in self.backbone.layer4:
            ...
            gate_num += 2
        output.append(x)
        return output,decision[:,gate_num:,:],bias[:,gate_num:,:]

    def forward(self, x, prompt=None):
        decision,bias=self.gate(prompt)
        gate=decision
        # x = self.backbone(x)
        x,decision,bias= self.forward_backbone(x, decision,bias)
        x = self.fpn(x)
        x_class = self.class_net(x,decision[:,0:3,:],bias[:,0:3,:])
        x_box = self.box_net(x,decision[:,3:6,:],bias[:,3:6,:])
        return x_class, x_box,gate
