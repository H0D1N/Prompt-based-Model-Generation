...
for file_number in range(1, 1001):
    ...
    try:
        with open(file_path, 'r') as file:
            ...
            if len(lines) >= 19:
                line = lines[18]  # 第19行在Python中索引为18
                avg_index = line.find("avg: ")  # 提取的是run avg, 单位us
                if avg_index != -1:
                    avg_value = line[avg_index + 5:].strip().split()[0]
                    avg_values_list.append(float(avg_value))
                else:
                    avg_values_list.append(None)
            else:
                avg_values_list.append(None)
...
#
model_latency_train = np.array(avg_values_list[:slice_num]).reshape(slice_num, 1)  # 将前800个元素作为训练集
model_latency_valid = np.array(avg_values_list[slice_num:]).reshape(runs - slice_num, 1)  # 将后200个元素作为测试集
...
with open("../onnx_models/configs.txt", 'r') as file:
