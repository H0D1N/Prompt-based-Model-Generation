...
model_memory_train = np.array(Self_CPU_Mem_values[:slice_num]).reshape(slice_num, 1)  # 将前700个元素作为训练集
model_memory_valid = np.array(Self_CPU_Mem_values[slice_num:]).reshape(runs - slice_num, 1)  # 将后300个元素作为测试集
